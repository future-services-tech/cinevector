...
export function CastGrid({ cast, awardsNote }: { cast: CastMember[]; awardsNote?: string }) {
  return (
    <div data-purpose="cast-and-crew-section">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        ...
        {awardsNote && (
          <div className="glass-card flex flex-col justify-center rounded-xl border border-amber-400/20 p-3 text-center">
            <span className="text-lg">🏆</span>
            <p className="mt-1 text-[11px] font-semibold text-amber-300">{awardsNote}</p>
          </div>
        )}
      </div>
    </div>
