...
export function RightDetailPanel() {
  ...
  const { detailOpen, openDetail, closeDetail } = useLayout();
  ...
  // Sotto lg il pannello è un drawer fuori schermo: selezionare un nodo (qui o sulla sfera) lo apre in
  // automatico, così l'utente non deve anche cercare un bottone separato per vedere cosa ha appena toccato.
  useEffect(() => {
    if (selectedId) openDetail();
  }, [selectedId, openDetail]);

  return (
    <>
      {detailOpen && (
        <div className="fixed inset-0 z-30 bg-black/50 lg:hidden" onClick={closeDetail} aria-hidden="true" />
      )}
      <aside
        data-purpose="right-details-panel"
        className={`scrollbar-thin fixed inset-y-0 right-0 z-40 flex w-84 shrink-0 flex-col overflow-y-auto border-l border-white/5 bg-space-850 transition-transform duration-200 md:w-96 lg:static lg:z-auto lg:translate-x-0 lg:bg-space-850/60 lg:transition-none ${
      ...
      >
        <div className="flex items-center justify-between border-b border-white/5 px-5 py-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={closeDetail} aria-label="Chiudi dettaglio" className="glass-pill rounded-full p-1.5 text-slate-300 hover:text-ink lg:hidden">
              <X size={15} />
            </button>
          </div>
        </div>
        ...
        {movie && (
          <div className="space-y-5 p-5">
            <div className="space-y-1.5">
              ...
              <h2 className="text-xl font-extrabold text-ink">{movie.title}</h2>
            </div>
