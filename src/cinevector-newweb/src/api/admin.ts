...
export function getSearchLatency(hours = 6): Promise<MetricSeries[]> {
  return apiFetch<MetricSeries[]>(`/api/admin/metrics/search-latency?hours=${hours}`);
}

export function getDbLatency(hours = 6): Promise<MetricSeries[]> {
  return apiFetch<MetricSeries[]>(`/api/admin/metrics/db-latency?hours=${hours}`);
}

export function getSearchAnalytics(hours = 24): Promise<SearchAnalytics> {
  return apiFetch<SearchAnalytics>(`/api/admin/search-analytics?hours=${hours}`);
}

export function getLogs(level?: string, take = 100): Promise<LogEntry[]> {
  const qs = new URLSearchParams({ take: String(take) });
  if (level) qs.set("level", level);
  return apiFetch<LogEntry[]>(`/api/admin/logs?${qs.toString()}`);
}

export function backfillEmbeddings(batchSize = 50): Promise<{ processed: number }> {
